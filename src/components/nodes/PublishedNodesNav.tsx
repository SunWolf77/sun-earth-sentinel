/**
 * Compact published-node strip for header chrome.
 * Focus in-app (no reload) · SES home clears focus · optional external board.
 * Detailed breakdown stays in sidebar FocusedNodeCard / NodeFocusPanel.
 */
...
export function PublishedNodesNav({ className = "" }: { className?: string }) {
  ...
  const anyFocused = monitors.some((m) => m.sesNodeId === focused);
  ...
  const goNode = (sesNodeId: string) => {
    ...
    if (focused === sesNodeId) {
      // Second tap on same chip = home (instant back to SES world)
      exitToHomeView();
      setMobileSheet("closed");
      return;
    }
  ...
  return (
    <div
      className={`ww-nodes-nav ${className}`}
      role="navigation"
    ...
    >
      <button
        ...
        title="Sun-Earth Sentinel home — full world view"
        ...
        return (
          <span key={m.sesNodeId} className="ww-nodes-nav__pair">
            <button
              ...
              title={`${m.name} · focus on map (SES #${m.networkOrder}). Tap again for home.`}
              aria-pressed={on}
            ...
            {board && (
              <a
                ...
                onClick={() => {
                  // Keep SES focused so return feels continuous
