...
export function LiveMap() {
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem("wolfwatch_gesture_tip_v1")) return;
    } catch {
      /* ignore */
    }
    // Let bootstrap settle before teaching gestures
    const showT = window.setTimeout(() => setShowGestureTip(true), 12_000);
    return () => window.clearTimeout(showT);
  }, []);

  useEffect(() => {
    if (!showGestureTip) return;
